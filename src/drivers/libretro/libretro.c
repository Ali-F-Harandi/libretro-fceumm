...
#else
#ifndef _GNU_SOURCE
/* Dl_info and dladdr() are GNU extensions on glibc; defining _GNU_SOURCE
 * before #include <dlfcn.h> exposes them. Must be defined before any
 * system header is pulled in, so we do it here in the autoload block
 * (before <dlfcn.h>) — the rest of the core has already pulled in its
 * headers by this point, but defining _GNU_SOURCE now is harmless for
 * them and necessary for dladdr() below. */
#define _GNU_SOURCE
#endif
#include <dlfcn.h>
#include <time.h>
...
static void autoload_logf(const char *fmt, ...)
{
   ...
   if (log_cb.log)
      log_cb.log(RETRO_LOG_INFO, "[autoload] %s\n", line);
 ...
 * "<archive>.zip#<inner-file>.nes" - we use the inner file name. */
